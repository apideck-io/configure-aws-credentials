...
describe("Configure AWS Credentials", () => {
  beforeEach(() => {
    ...
    core.getIDToken = jest.fn().mockImplementation(() => {
      return "testtoken";
    });
  ...
  test("action fails when github env vars are not set", async () => {
    ...
    core.getInput = jest.fn().mockImplementation(mockGetInput(ASSUME_ROLE_INPUTS));
  ...
  test("action with no accessible credentials fails", async () => {
    ...
    expect(core.setFailed).toHaveBeenCalledWith(
      "Credentials could not be loaded, please check your action inputs: No credentials to load",
    );
  });
  ...
  test("throws error if access key id exists but missing secret access key", async () => {
    ...
    core.getInput = jest.fn().mockImplementation(mockGetInput(inputsWIthoutSecretKey));
  ...
  test("role assumption duration provided", async () => {
    core.getInput = jest.fn().mockImplementation(mockGetInput({ ...ASSUME_ROLE_INPUTS, "role-duration-seconds": 5 }));
  ...
  test("role external ID provided", async () => {
    core.getInput = jest.fn().mockImplementation(mockGetInput({ ...ASSUME_ROLE_INPUTS, "role-external-id": "abcdef" }));
